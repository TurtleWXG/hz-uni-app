...
var _vuex = _interopRequireDefault(__webpack_require__(/*! vuex */ 12));function _interopRequireDefault(obj) {return obj && obj.__esModule ? obj : { default: obj };} // 此为vue状态管理页面(使用模块管理的前提:父组件下有多个子组件，子组件之间的通讯就会非常繁琐，父组件也需要监听大量的事件)
// 在Vue中使用Vuex
...
var store = new _vuex.default.Store({
  ...
  mutations: {
    ...
    // |更新扫码放行信息
    // +------------------
    refreshScanPersonData: function refreshScanPersonData(state, data) {
      state.personScanPersonData = data;
    },
    // +------------------
    // |更新个人信息
    // +------------------
    refreshPersonData: function refreshPersonData(state, data) {
      state.personData = data;
    },
    // +------------------
    // |更新openid
    // +------------------
    refreshOpenId: function refreshOpenId(state, openId) {
      state.openId = openId;
    },
    // +------------------
    // |更新用户权限
    // +------------------
    refreshUserType: function refreshUserType(state, index) {
      state.userType = index;
    },
    // +------------------
    // |更新登记信息
    // +------------------
    refreshRegisterData: function refreshRegisterData(state, data) {
      state.registerData = data;
    },
    // +------------------
    // |更新登记成功状态
    // +------------------
    refreshRegisterStatus: function refreshRegisterStatus(state, status) {
  ...
  actions: {
    ...
    // |更新扫码放行信息
    // +------------------
    refreshScanPersonData: function refreshScanPersonData(context, data) {
      context.commit('refreshScanPersonData', data);
    },
    // +------------------
    // |更新个人信息
    // +------------------
    refreshPersonData: function refreshPersonData(context, data) {
      context.commit('refreshPersonData', data);
    },
    // +------------------
    // |更新openid
    // +------------------
    refreshOpenId: function refreshOpenId(context, openId) {
      context.commit('refreshOpenId', openId);
    },
    // +------------------
    // |更新用户类型
    // +------------------
    refreshUserType: function refreshUserType(context, index) {
      context.commit('refreshUserType', index);
    },
    // +------------------
    // |更新登记信息
    // +------------------
    refreshRegisterData: function refreshRegisterData(context, data) {
      context.commit('refreshRegisterData', data);
    },
    // +------------------
    // |更新登记成功状态
    // +------------------
    refreshRegisterStatus: function refreshRegisterStatus(context, status) {
...
var _system = _interopRequireDefault(__webpack_require__(/*! ./system */ 20));

var _Json = _interopRequireDefault(__webpack_require__(/*! ./Json */ 22));var _this = void 0;function _interopRequireDefault(obj) {return obj && obj.__esModule ? obj : { default: obj };} //---------------------测试用数据-------------
...
module.exports = {
  ...
  config: function config(name) {
    ...
    if (name) {
      ...
      if (info == null) {
        var web_config = _system.default.get("web_config");
        if (web_config) {
          if (name2.length > 1) {
...
// +-----------------------------------
// 项目路径 h5为项目名
// const ROOTPATH = "http://127.0.0.1:8082/webfhj/";
// const ROOTPATH = "http://192.168.1.172:8082/webfhj/";
// const ROOTPATH = "https://14.116.205.228:8082/newwebsas/";
var ROOTPATH = "https://www.toonan.com/webpass/";
// const ROOTPATH = "https://www.toonan.com/webfhj/";

module.exports = {
  APIHOST: ROOTPATH + "sas/",
  IMAGE_PATH: ROOTPATH };
...
var _custom = _interopRequireDefault(__webpack_require__(/*! ./custom */ 21));var _module$exports;function _interopRequireDefault(obj) {return obj && obj.__esModule ? obj : { default: obj };}function _defineProperty(obj, key, value) {if (key in obj) {Object.defineProperty(obj, key, { value: value, enumerable: true, configurable: true, writable: true });} else {obj[key] = value;}return obj;} /**
                                                                                                                                                                                                                                                                                                                                                                                       * 缓存数据优化
...
module.exports = (_module$exports = { /**
                                       ...
                                       * @param v [键值]
                                       * @param t [时间、单位秒]
                                       */set: function set(k, v, t) {uni.setStorageSync(k, v);var seconds = parseInt(t);if (seconds > 0) {var timestamp = Date.parse(new Date());timestamp = timestamp / 1000 + seconds;uni.setStorageSync(k + postfix, timestamp + "");} else {uni.removeStorageSync(k + postfix);}}, /**
                                                                                                                                                                                                                                                                                                                        * 获取缓存 
                                                                                                                                                                                                                                                                                                                        * @param k  [键名]
                                                                                                                                                                                                                                                                                                                        * @param v  [获取为空时默认]
                                                                                                                                                                                                                                                                                                                        */get: function get(k, v) {var deadtime = parseInt(uni.getStorageSync(k + postfix));if (deadtime) {if (parseInt(deadtime) < Date.parse(new Date()) / 1000) {if (v) {return v;
        } else {
          return false;
        }
      }
  ...
  // |@cb 回调函数
  // +-------------------------------------
  chooseImage: function chooseImage(oldImgList, maxImgTotal, cb) {
    uni.chooseImage({
      ...
      success: function success(res) {
        // 判断图片是否大于2M
        var size = parseInt(res.tempFiles[0].size) / 1024 / 1024;
        if (size >= 2) {
          cb([], []);
          return false;
        }
        // 新上传的图片数据
        var newImgList = res.tempFilePaths;
        // 新上传的图片数量
        var nowLenght = newImgList.length;
        // 已上传的图片数量
        var imageLenght = oldImgList.length;
        // 如果已上传的图片数量小于允许上传图片数量
        if (imageLenght <= maxImgTotal) {
          var images = [];
          // 获取缺少的图片张数
          var residue = maxImgTotal - imageLenght;
          // 如果缺少的张数大于当前的的张数  
  ...
  showLoad: function showLoad(title) {
    if (_custom.default.isEmpty(title)) title = '加载中';
    console.log("231123");
    uni.showLoading({
      mask: true,
      title: title });

  } }, _defineProperty(_module$exports, "showLoad", function showLoad(



title) {
  uni.hideLoading();
}), _defineProperty(_module$exports, "getBackPage", function getBackPage()
...
module.exports = {
  ...
  /*
     ...
     * @param basic [数组/字符串]
     */
  clearRepeatData: function clearRepeatData(basic) {
  ...
  /*
     ...
     */
  sort: function sort(basic, index) {
    switch (index) {
      // 数字排序
      case 1:
  ...
  /*
     ...
     */
  toUpperCase: function toUpperCase(basic, flag) {
  ...
  /*
     ...
     */
  getRandom: function getRandom(length, index) {
    var result = [];
    switch (index) {
      case 1:
        str = '1234567890';
  ...
  /*
     ...
     */
  getIDCard: function getIDCard(basic, separator) {
...
module.exports = {
  ...
  /**
      ...
      * @param path [路径]
      */
  navBack: function navBack() {var level = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : 1;
    return new Promise(function (resolve) {
      uni.navigateBack({
        delta: level });
...
"use strict";
/* WEBPACK VAR INJECTION */(function(uni) {Object.defineProperty(exports, "__esModule", { value: true });exports.default = void 0;var uQRCode = {};
...
"use strict";
/* WEBPACK VAR INJECTION */(function(uni) {
var _config = _interopRequireDefault(__webpack_require__(/*! ./config.js */ 19));function _interopRequireDefault(obj) {return obj && obj.__esModule ? obj : { default: obj };} // uni-app请求封装
module.exports = {
  http: function http(router) {var data = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : {};var method = arguments.length > 2 ? arguments[2] : undefined;
    // 基础地址
    var path = _config.default.APIHOST + 'passLink/';
    // 返回promise
    return new Promise(function (resolve, reject) {
      // 请求
      uni.request({
        url: "".concat(path).concat(router),
        data: data,
        method: method,
        header: {

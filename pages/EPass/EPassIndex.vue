<template>
	<view>
		<scroll-view scroll-y class="DrawerPage" :class="modalName=='viewModal'?'show':''">
			<form @submit="formSubmit" @reset="formReset">
				...
				<view class="cu-form-group">
					...
					<input v-model="zjhm" type="idcard" name="zjhm" placeholder="请输入"></input>
				...
				<view class="cu-form-group">
					...
					<picker @change="bindDropChange" data-name="sqdx" range-key="name" :range="sqdxList">
						<view class="picker">
		...
		<scroll-view scroll-y class="DrawerWindow" :class="modalName=='viewModal'?'show':''">
			...
			<view class="uni-common-mt" v-if="status === 'jddm'">
				<view class="uni-form-item uni-column">
					<input class="uni-input search-input" name="jddmName" placeholder="搜索街路巷" @confirm="jddmSearchConfirm" />
				</view>
			</view>
		...
		methods: {
			...
			async getPersonByZjhm() {
				...
				const {
					data,
					appCode
				} = await this.$request.post(this.$api.getPersonByZjhm, {
					"zjhm": this.zjhm
				});
				uni.hideLoading();
				if (appCode == 1) {
